// --------------------------------------------------------------------------------------------------------------------
// <copyright file="TaxonomyMenuPartDriver.cs" company="Zaust">
                ...
                () =>
                    {
                        ...
                        {
                            var allTerms =
                                _taxonomyService.GetTerms(taxonomy.Id).Select(
                                    (t, i) => new { Level = t.GetLevels(), Term = t, Index = i }).ToList();
                ...
                () =>
                    {
                        ...
                        return shapeHelper.EditorTemplate(
                            TemplateName: "Parts/Taxonomies.Menu", Model: model, Prefix: Prefix);
                    });
        }
        ...
        protected override void Exporting(TaxonomyMenuPart part, ExportContentContext context)
        {
            ...
            if (part.TaxonomyPartRecord != null)
            {
                ...
                context.Element(part.PartDefinition.Name).SetAttributeValue(
                    "TaxonomyPartRecordId", _contentManager.GetItemMetadata(taxonomy).Identity);
            }
            if (part.TermPartRecord != null)
            {
                var term = _taxonomyService.GetTerm(part.TermPartRecord.Id);
                context.Element(part.PartDefinition.Name).SetAttributeValue(
                    "TermPartRecordId", _contentManager.GetItemMetadata(term).Identity);
            }
        }
