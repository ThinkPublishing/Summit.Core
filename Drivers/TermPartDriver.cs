// --------------------------------------------------------------------------------------------------------------------
// <copyright file="TermPartDriver.cs" company="Zaust">
...
namespace Summit.Core.Drivers
{
    ...
    public class TermPartDriver : ContentPartDriver<TermPart>
    {
        ...
        public TermPartDriver(
            ITaxonomyService taxonomyService, IContentManager contentManager, ITermPathConstraint termPathConstraint)
        {
        ...
        protected override DriverResult Editor(TermPart termPart, IUpdateModel updater, dynamic shapeHelper)
        {
            if (updater.TryUpdateModel(termPart, Prefix, null, null))
            {
                ...
                if (existing != null && existing.Record != termPart.Record
                    && existing.Container.ContentItem.Record == termPart.Container.ContentItem.Record)
                {
        ...
        protected override void Exporting(TermPart part, ExportContentContext context)
        {
            ...
            context.Element(part.PartDefinition.Name).SetAttributeValue(
                "Path", String.Join(",", identityPaths.ToArray()));
        ...
        protected override void Importing(TermPart part, ImportContentContext context)
        {
            ...
            foreach (
                var identityPath in
                    context.Attribute(part.PartDefinition.Name, "Path").Split(
