// --------------------------------------------------------------------------------------------------------------------
// <copyright file="TermsPartHandler.cs" company="Zaust">
...
namespace Summit.Core.Handlers
{
    ...
    public class TermsPartHandler : ContentHandler
    {
                ...
                (context, part) =>
                    {
                        foreach (var field in part.ContentItem.Parts.SelectMany(p => p.Fields).OfType<TaxonomyField>())
                        {
                            ...
                            var fieldTermRecordIds =
                                part.Record.Terms.Where(t => t.Field == tempField).Select(tci => tci.TermRecord.Id);
                            field.Terms.Loader(
                                value => fieldTermRecordIds.Select(id => _contentManager.Get<TermPart>(id)));
                        }
                    });
        ...
        private static void RecalculateCount(
            IContentManager contentManager, ITaxonomyService taxonomyService, TermsPart part)
        {
        ...
        protected override void Activating(ActivatingContentContext context)
        {
            ...
            if (
                contentTypeDefinition.Parts.Any(
                    part =>
                    part.PartDefinition.Fields.Any(field => field.FieldDefinition.Name == typeof(TaxonomyField).Name)))
            {
