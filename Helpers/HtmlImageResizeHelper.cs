// --------------------------------------------------------------------------------------------------------------------
// <copyright file="HtmlImageResizeHelper.cs" company="Zaust">
...
namespace Summit.Core.Helpers
{
    ...
    public static class HtmlImageResizeHelper
    {
            ...
            object htmlAttributes = null)
        {
            ...
            {
                if (width > 0) sb.AppendFormat("width:{0}px; ", width);
                if (height > 0) sb.AppendFormat("height:{0}px; ", height);
            }
            var attributes = HtmlHelper.AnonymousObjectToHtmlAttributes(htmlAttributes);

            var attrBuilder = new StringBuilder();
            attributes.IfNotNull(
                x =>
                x.ToList().ForEach(
